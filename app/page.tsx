    ...
    return (
            <main className={`max-w-5xl mx-auto flex flex-col gap-6`}>
                <header className={`flex gap-4 justify-center`}>
                    <h1 className={`text-5xl font-extrabold text-lime-300 text-center`}>NexsmaJS</h1>
                ...
                <section className={`grid gap-5 grid-cols-1 lg:grid-cols-2`}>
                    {posts.map((post) => (
                        <div key={post.id} className={`flex flex-col gap-3 p-5 bg-stone-800 rounded`}>
                            <Link href={`/posts/${post.id}`} className={`text-2xl font-bold text-orange-400 hover:underline`}>
                                {post.title}
                            </Link>
                            <div className={`font-light`}>
                                <p className={`max-h-[7.5ch]  overflow-hidden`}>
                                    {trimText(post.content, 24)}
                                    ...
                                </p>
                            </div>
